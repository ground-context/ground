...
public class WikiExtractor implements Extractor<String, JsonElement>{
  
  private static final Logger LOG = LoggerFactory.getLogger(WikiExtractor.class);
  ...
  private class WikiResponseReader implements Iterator<JsonElement> {
    ...
    public boolean hasNext() {
      if (!WikiExtractor.this.recordsOfCurrentTitle.isEmpty()) {
        return true;
      } else if (WikiExtractor.this.requestedTitles.isEmpty()) {
        return false;
      } else {
         ...
         */
        while (!WikiExtractor.this.requestedTitles.isEmpty()) {
          String currentTitle = WikiExtractor.this.requestedTitles.poll();
          try {
            WikiExtractor.this.recordsOfCurrentTitle = retrievePageRevisions(currentTitle);
          } catch (IOException e) {
    ...
    public JsonElement next() {
      ...
      return WikiExtractor.this.recordsOfCurrentTitle.poll();
    }
  ...
  public WikiExtractor(WorkUnitState workUnitState) throws IOException {
