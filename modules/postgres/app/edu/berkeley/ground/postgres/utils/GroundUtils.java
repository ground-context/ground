...
public final class GroundUtils {
    ...
    public static ObjectNode getServerError(final Request request, final Throwable e) {
        Logger.error("error: Path: {}  Message: {} Trace: {}", request.path(), e.getMessage(), e.getStackTrace());
        ObjectNode result = Json.newObject();
        result.put("groundExceptionMessage", "ERROR: Processing Request");
        result.put("message", String.format("Path:%s Trace: %s", request.path(), e.getStackTrace().toString()));
        return result;
    }
