...
public class LineageGraphController extends Controller {
  ...
  private CassandraLineageGraphVersionDao cassandraLineageGraphVersionDao;

  @Inject
  final void injectUtils(final CacheApi cache, final CassandraDatabase dbSource,
    ...
    this.cassandraLineageGraphVersionDao = new CassandraLineageGraphVersionDao(dbSource, idGenerator);
  }
      ...
      () -> {
        try {
          return this.cache.getOrElse(
            "lineage_graphs",
            () -> Json.toJson(this.cassandraLineageGraphDao.retrieveFromDatabase(sourceKey)),
      ...
      () -> {
        try {
          return this.cache.getOrElse(
            ...
            () -> Json.toJson(this.cassandraLineageGraphVersionDao.retrieveFromDatabase(id)),
            Integer.parseInt(System.getProperty("ground.cache.expire.secs")));
        } catch (Exception e) {
          throw new CompletionException(e);
        }
  ...
  public final CompletionStage<Result> createLineageGraph() {
    return CompletableFuture.supplyAsync(
      () -> {
        ...
        try {
          lineageGraph = this.cassandraLineageGraphDao.create(lineageGraph);
  ...
  public final CompletionStage<Result> createLineageGraphVersion() {
    return CompletableFuture.supplyAsync(
      () -> {
        ...
        try {
          lineageGraphVersion = this.cassandraLineageGraphVersionDao.create(lineageGraphVersion, parentIds);
        } catch (GroundException e) {
          throw new CompletionException(e);
        }
