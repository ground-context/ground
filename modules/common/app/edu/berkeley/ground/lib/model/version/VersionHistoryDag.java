...
public class VersionHistoryDag<T extends Version> {
   ...
   *
   * @param itemId
   *          the id of the item of this DAG
   * @param edges
   *          the version successors in this DAG
   */
   ...
   *
   * @param parentId
   *          the id of the "from" of the edge
   * @param childId
   *          the id of the "to" of the edge
   */
  ...
  public List<Long> getParent(long childId) {
    return this.parentChildMap.entrySet().stream()
        .filter(entry -> entry.getValue().contains(childId)).map(Map.Entry::getKey)
