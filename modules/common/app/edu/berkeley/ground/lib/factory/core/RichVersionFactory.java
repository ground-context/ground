...
public interface RichVersionFactory<T extends RichVersion> extends VersionFactory<T> {
    ...
    void insertIntoDatabase(long id, Map<String, Tag> tags, long structureVersionId, String reference,
     ...
     *            the provided tags
     */
    static void checkStructureTags(StructureVersion structureVersion, Map<String, Tag> tags) throws GroundException {
	...
	for (String key : structureVersionAttributes.keySet()) {
		...
		throw new GroundException("Tag with key " + key
			+ " did not have a value of the correct type: expected [" + structureVersionAttributes.get(key)
			+ "] but found [" + tags.get(key).getValueType() + "].");
	    }
	}
    }
}
